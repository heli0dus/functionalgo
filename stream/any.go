...
func (s Stream) Any(f interface{}) Stream {
	if !s.Valid() {
		return s
	}
	...
	if fType.Kind() != reflect.Func {
		return s.Error(fmt.Errorf("in Any expected function, but got %v", fType))
	}

	// return type check
	if !(fType.Out(0).Kind() == reflect.Bool && (fType.NumOut() == 1 || (fType.NumOut() == 2 && fType.Out(1).Implements(reflect.TypeFor[error]())))) {
		return s.Error(fmt.Errorf("function used in Any must return bool or (bool, error)"))
	}

	// param type check
	if fType.NumIn() != 1 {
		return s.Error(fmt.Errorf("function used in Any must receive 1 argument"))
	}
